...
def create_unified_sale_from_pos(sender, instance, created, **kwargs):
    """Create unified sales record when POS sale is created"""
    if created and instance.status == 'completed':
        ...
        unified_sale = Sales.objects.create(
            ...
            change_given=instance.change_given,
...
def create_unified_sale_from_ecommerce(sender, instance, created, **kwargs):
    """Create unified sales record when e-commerce order is created"""
    if created and instance.status in ['confirmed', 'processing', 'shipped', 'delivered']:
